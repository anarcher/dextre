...
func Nodes(kubectl *kubernetes.Client, instanceGroup InstanceGroup, cluster string, awsRegion string, verbose bool) error {
	...
	for _, node := range nodes.Items {
		if node.Labels[instanceGroup.NodeLabel] == instanceGroup.Name {
	...
	for _, node := range rollableNodes {
		...
		asg, err := client.GetAutoScalingGroup(
			instanceGroup.AutoScalingGroupLabel,
			instanceGroup.Name, cluster)
		if err != nil {
			return err
		}
...
//Run: executes the drain command
func DeleteCordonNodes(kubectl *kubernetes.Client,
	awsRegion string,
	skipDrain, dryRun, verbose bool) error {
	spinner := spinner.New(spinner.CharSets[9], 100*time.Millisecond)
	...
	for _, node := range rollableNodes {
		...
		if skipDrain == false {
			...
			ui.Print(fmt.Sprintf("[-] Skip drain node: %s", node.Name), true)
		}
